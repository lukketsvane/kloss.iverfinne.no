...
/**
 * Launch / dash whoosh: a burst of noise through a rising bandpass sweep.
 * @param strength 0..1 – pull strength / dash intensity
 */
export function playWhoosh(strength: number) {
  if (muted) return
  const c = ensureCtx()
  if (!c || !master || c.state !== "running") return
  const now = c.currentTime
  ...
  const bp = c.createBiquadFilter()
  bp.type = "bandpass"
  bp.frequency.setValueAtTime(240 + v * 300, now)
  bp.frequency.exponentialRampToValueAtTime(900 + v * 1600, now + 0.16)
  bp.Q.value = 1.4
  const g = c.createGain()
  g.gain.setValueAtTime(0.0001, now)
  g.gain.exponentialRampToValueAtTime(0.10 + v * 0.22, now + 0.03)
...
/** Deep shockwave boom for the orange block's blast. */
export function playBoom() {
  if (muted) return
  const c = ensureCtx()
  if (!c || !master || c.state !== "running") return
  const now = c.currentTime
  // low sine drop – the body of the boom
  const o = c.createOscillator()
  o.type = "sine"
  ...
  o.start(now)
  o.stop(now + 0.45)
  // and a noise slap on top
  const noise = c.createBufferSource()
  noise.buffer = noiseBuffer(c)
  noise.playbackRate.value = 0.4
...
/** Bright knock-down pop when a knot is defeated. */
